...
export const getBatchesForClass = async (req, res) => {
  try {
    ...
    const batches = await getDistinctBatchesByClass(classNumber);
...
export const getStudentsByClassBatch = async (req, res) => {
  try {
    ...
    const students = await getStudentsByClassAndBatch(classNumber, batch);
...
export const generateReportCardPDF = async (req, res, next) => {
  try {
    ...
    if (!admission_no || !term) {
      console.log("❌ Missing parameters for PDF:", { admission_no, term });
      return res.status(400).json({
        error: "Missing required parameters: admission_no and term"
      });
    }

    console.log(`🚀 Starting PDF generation for ${admission_no}, term: ${term}`);
